...
public class StateGraph implements Node{
    ...
    private Map<String, Node> getObjectStringMap() {
        ...
        retrieverToDescription.put("数据库混合检索节点，用于从数据库中检索信息，使用混合检索的方法，适合通用情况", milvusHybridRetrieveNode);
        retrieverToDescription.put("数据库过滤检索节点，用于从数据库中检索信息，先用关键词进行过滤，再向量检索信息", milvusFilterRetrieveNode);
        retrieverToDescription.put("数据库标题检索节点，用于从数据库中检索信息，检索标题相同的文档，适合问题中提及文档名称的情况", milvusQueryRetrieveNode);
