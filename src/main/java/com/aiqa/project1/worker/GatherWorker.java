...
public class GatherWorker {
    ...
    public void receiveRetrieveResult(State state) {
        ...
        Long retrievalCount = redisStoreUtils.decreaseRetrievalCount(state.getUserId(), state.getSessionId(), state.getMemoryId(), state.getParams());
        
        if (retrievalCount <= 0) {
            rabbitTemplate.convertAndSend("retrievalInformation.summary", state);
        } else {
